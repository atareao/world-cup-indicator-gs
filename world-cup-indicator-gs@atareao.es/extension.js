...
class Ateam extends St.BoxLayout{
    constructor(tname=null, ticon_name=null){
		...
		let ficon = Extension.path + '/icons/' + ticon_name;
		let ficonp = Gio.File.new_for_path(ficon + '.png');
		var icon;
		if (ficonp.query_exists(null)){
			icon = Gio.icon_new_for_string(ficon + '.png');
		}else{
			icon = Gio.icon_new_for_string(ficon + '.svg');
		}
        this.team_icon = new St.Icon({gicon: icon,
                                      icon_size: 48,
                                      y_align: Clutter.ActorAlign.CENTER,
                                      x_align: Clutter.ActorAlign.CENTER});
...
let WorldCupIndicator = GObject.registerClass (
	class WorldCupIndicator extends PanelMenu.Button{
	    _init(){
			...
			let menu_icon = Gio.icon_new_for_string(Extension.path + '/icons/world-cup-indicator-gs.svg')
	
	        let icon = new St.Icon({ gicon: menu_icon,
	        ...
	        this.sourceId = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT,
	                                                 60, // every minute
	                                                 this._updater.bind(this));
	    }
	
	    _updater(force=false){
	        let ahora = new Date();
	        this.updateCurrent();
